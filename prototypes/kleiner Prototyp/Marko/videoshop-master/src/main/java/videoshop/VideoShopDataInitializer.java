...
public class VideoShopDataInitializer implements DataInitializer {
	...
	private void initializeUsers(UserAccountManager userAccountManager, CustomerRepository customerRepository) {
		...
		// Test mit Accountverwaltung
		UserAccount ub1 = userAccountManager.create("NewCustomer", "slave", customerRole);
		userAccountManager.save(ub1);
		Customer cb1 = new Customer(ub1, "Street");
		customerRepository.save(cb1);
		
		UserAccount ub2 = userAccountManager.create("NewCustomer02", "slave", customerRole);
		userAccountManager.save(ub2);
		Customer cb2 = new Customer(ub2, "Amalie d.p. 12" , "01169", "Dresden", "NC02@web.de", "male");
		customerRepository.save(cb2);
		cb2.setAddress("Amalie d.p. 2");
		cb2.setcommune("dresden");
		cb2.setemail("none@web.de");
		cb2.setgender("female");				
		
		customerRepository.delete(cb2);		
